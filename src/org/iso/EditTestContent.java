...
public class EditTestContent extends HttpServlet {
	...
	public void doGet (HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String sql = "Select Description, NumberOfQuestions, NumberOfOptions FROM Tests WHERE Id_Tests=" + id +"";
		...
		while(result.next()) {
		testName = result.getString("Description");
		...
		String sqlz = "Select OptionT FROM Options WHERE IdTest=" + id + "";
		int dim = num * op;
		...
		String sqlo = "Select IdQuestion, QuestionT, Answer FROM Questions WHERE IdTest=" + id +"";
		...
		while(result.next()) {
		position = result.getString("IdQuestion");
		pos = Integer.parseInt(position);
		quest = result.getString("QuestionT");
		...
		for(int j=0; j<op; j++) {
				...
				int w = j + 1;
				out.println("<tr><td><input type='radio' name='" + pos + "' value=" + w + "></td><td><h6>Option " + w + ":</h6></td><td><INPUT type='text' name='option" + pos + w + "' value='" + Opciones[conti] + "'></td>");
				conti = conti + 1;
